...
function encrypt(masterKey, data) {
    ...
    let cipherPayload = crypto.createCipher('aes-256-gcm', key);
    let cipherPayloadLen = crypto.createCipher('aes-256-gcm', key);
    let payload = cipherPayload.update(data);
    cipherPayload.final();
    let payloadTag = cipherPayload.getAuthTag();
    let buf = buffer_1.Buffer.alloc(2);
    ...
    let encrypted = buffer_1.Buffer.concat([salt, payloadLen, payloadLenTag, payload, payloadTag]);
    return encrypted;
}
function hkdfTest() {
    let salt = buffer_1.Buffer.from("123456789012");
    let subKey = futoin_hkdf_1.default("evenardo", 32, { salt: salt, info: "ss-subkey", hash: "SHA-1" });
    console.log(subKey);
}
// hkdfTest();
function decryptShadow(masterKey, salt, nonce, encryptedBuffer) {
...
function test() {
    ...
    let data = "Hello2";
    console.time("start");
    let encrypted = encrypt(key, buffer_1.Buffer.from(data));
...
function EVP_BytesToKey(password, key_len, iv_len) {
    ...
    while (count < key_len + iv_len) {
        ...
        data = password;
        if (i > 0) {
            data = buffer_1.Buffer.concat([m[i - 1], password]);
        }
        md5.update(data);
        d = md5.digest();
        m.push(d);
...
console.log("haha");
let evpBytesToKey = EVP_BytesToKey(buffer_1.Buffer.from("evenardo", "binary"), 32, 32);
